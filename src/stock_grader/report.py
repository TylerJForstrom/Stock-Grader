...
__all__ = ["render_report", "render_ranking", "render_consensus", "to_json", "to_markdown"]
...
def render_report(report: GradeReport, console: Console | None = None, *, explain: bool = False) -> None:
    ...
    if report.ci and report.ci[0] == report.ci[0]:
...
def render_ranking(reports: dict[str, GradeReport], console: Console | None = None, *, top: int | None = None) -> None:
    ...
    ordered = sorted(
        reports.values(),
        key=lambda r: (r.letter == "N/A", -r.score if r.score == r.score else 1e9),
    )
    if top:
        ordered = ordered[:top]
    for i, report in enumerate(ordered, 1):
        ci = f"{report.ci[0]:.0f}–{report.ci[1]:.0f}" if report.ci and report.ci[0] == report.ci[0] else "—"
        table.add_row(
            str(i),
            report.ticker,
...
def render_consensus(results: dict, console: Console | None = None) -> None:
    ...
    for result in sorted(results.values(), key=lambda r: -r.score if r.score == r.score else 1e9):
