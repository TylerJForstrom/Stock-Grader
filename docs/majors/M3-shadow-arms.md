...
- market_eod on-disk layout: <vault_dir>/market_eod/YYYY-MM/YYYY-MM-DD.jsonl.gz, written by _day_path(). Each line is json.dumps(row, sort_keys=True) with EXACT keys: symbol, open, high, low, close, volume, vwap, transactions — all numeric except `symbol`, and `volume` is a FLOAT. One whole-market row per symbol per session. (Sample rows and row counts are vendor data and stay in Stock-Vault.)  
...
- market_eod uses POLYGON DOT symbology, matching _to_broker_symbol. Verified against the archive: the dot form ('BRK.B', 'MOG.A') is present and the dash form ('BRK-B') is absent, and dotted class shares are a small but non-trivial slice of the tape. 'SPY' and 'VOO' are both present, so a benchmark series needs no new collector. (Closes and symbol counts are vendor data and stay in Stock-Vault.)  
