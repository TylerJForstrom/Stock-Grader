...
**Why it matters:** Turn the vault's four restricted-license archives (IB borrow, FINRA short interest, Finnhub recommendations, SSGA holdings) from write-only archives into evaluable signals by emitting panels that satisfy the exact column contract `stock_grader.backtest._validate_panel` enforces, so `stock-grader backtest <panel.parquet>` runs on them unchanged. The FINRA panel is evaluable TODAY (measured in the vault: a large cross-section survives the liquidity filters at each settlement, and the market_eod archive already spans enough sessions that widening the FINRA `--since` yields dozens of non-overlapping periods immediately — the counts themselves stay in Stock-Vault); the other three accrue forward. This is the first place in the ecosystem where a non-fundamental signal can be measured against an out-of-sample cross-section, which is the whole point of the edge hunt.
...
- IB borrow rows have exactly these keys: available, available_capped, con_id, currency, fee_rate, isin, name, rebate_rate, symbol. `fee_rate` is an annualized PERCENT (not a fraction) and is nullable; `currency` is predominantly USD with a small non-USD tail; `available_capped` marks rows whose availability is a ceiling rather than a quantity. Count the rows and the fee-rate distribution in the vault before using either. *(measured values live in Stock-Vault's private notes — this repo is PUBLIC and vault-derived numbers may not be published here)*  
  *(`Stock-Vault/data/borrow/2026-07/usa_20260729T2317.jsonl.gz; parser at src/stock_vault/borrow.py:parse_usa_txt`)*
- IB uses a SPACE class-share form: 'BRK A', 'BRK B', 'AAC U', 'ABR PRD'. market_eod (Polygon/Massive) uses the DOT form: 'BRK.A', 'BRK.B'. SEC/canonical uses the DASH form 'BRK-B'. All three appear in hundreds of names, so a join that ignores the separator forms silently drops class shares.  
...
- FINRA CSV is pipe-delimited with header: accountingYearMonthNumber|symbolCode|issueName|issuerServicesGroupExchangeCode|marketClassCode|currentShortPositionQuantity|previousShortPositionQuantity|stockSplitFlag|averageDailyVolumeQuantity|daysToCoverQuantity|revisionFlag|changePercent|changePreviousNumber|settlementDate. `marketClassCode` distinguishes OTC from listed venues (OTC is the single largest class and must be dropped); `stockSplitFlag` and `revisionFlag` are set on a small minority of rows and both must be handled. *(measured values live in Stock-Vault's private notes — this repo is PUBLIC and vault-derived numbers may not be published here)*  
  *(`Stock-Vault/data/finra_short_interest/shrt20260715.csv (measured)`)*
- FINRA's own daysToCoverQuantity is FLOORED at 1.00 and CAPPED at 999.99 (a zero-ADV row reports the cap; illiquid names pile up on the floor). It is unusable as a continuous cross-sectional variable.  
...
- Finnhub recs record shape is {"ticker": <requested>, "snapshot_month": "YYYY-MM", "rows": [{"buy","hold","period","sell","strongBuy","strongSell","symbol"}, …]} with exactly 4 monthly periods, newest == first of snapshot month. Coverage is a small ticker list, and only ONE snapshot month exists on disk — nowhere near a cross-section.  
  *(`Stock-Vault/data/rec_trends/finnhub_2026-07.jsonl.gz (measured)`)*
- DATA-INTEGRITY TRAP: for requested ticker 'BRK-B' Finnhub returned rows whose inner symbol field is 'BRK.A' — the wrong security. Always compare `rows[].symbol` against the requested ticker and drop the mismatches; do not assume the vendor echoed your request.  
  *(`Stock-Vault/data/rec_trends/finnhub_2026-07.jsonl.gz (measured)`)*
- SSGA XLSX layout (openpyxl 0-based rows): sheet name 'holdings'; row0 ('Fund Name:', <name>); row1 ('Ticker Symbol:', 'SPY'); row2 ('Holdings:', 'As of 28-Jul-2026') — the as-of date, one day BEFORE the directory date 2026-07-29; row3 blank; row4 header ('Name','Ticker','Identifier','SEDOL','Weight','Sector','Shares Held','Local Currency'); data from row5. 'Identifier' is the CUSIP. 12 funds: spy, xlk, xlf, xle, xlv, xli, xlp, xly, xlu, xlb, xlre, xlc. Only ONE snapshot directory exists (2026-07-29).  
...
- market_eod rows have keys: symbol, open, high, low, close, volume, vwap, transactions, one gzipped file per session under monthly manifests. The free tier is a rolling ~730-day archive, so the earliest sessions roll off — never assume a window stays covered. Collected with adjusted=false, i.e. UNADJUSTED for splits and dividends.  
  *(`Stock-Vault/src/stock_vault/market_eod.py:fetch_day (params adjusted="false"); data/market_eod/ (measured)`)*
- JOIN VIABILITY (measured in the vault, numbers NOT reproduced here): squashing all separators ('.', '-', ' ') from market_eod symbols yields a collision-free key for a whole session, and that key recovers the class shares an exact-symbol join loses in both the FINRA and the borrow feeds. Re-measure before relying on it. *(measured values live in Stock-Vault's private notes — this repo is PUBLIC and vault-derived numbers may not be published here)*  
...
- PANEL FEASIBILITY (measured in the vault, numbers NOT reproduced here): with filters close>=$5 and close*volume>=$1M a FINRA DTC panel is large enough to evaluate, a substantial minority of rows have no resolvable CIK, and the realized forward-return range contained no |return|>1. Row counts, the DTC distribution and the return range are a vault-derived EVALUATION RESULT and stay in Stock-Vault. *(measured values live in Stock-Vault's private notes — this repo is PUBLIC and vault-derived numbers may not be published here)*  
...
Pure module, no deps. Canonical form = SEC dash form, uppercase, stripped.
  ...
  def build_squash_index(symbols: Iterable[str]) -> tuple[dict[str, str], set[str]]:
      returns (key -> single market_eod symbol, ambiguous_keys). Build defaultdict(set) keyed by squash(); any key mapping to >1 symbol is EXCLUDED from the index and returned in ambiguous_keys so callers can count/report it. Measured in the vault today: zero ambiguity for a whole session — but the guard must exist because ticker reuse will eventually break it.
...
C) borrow_fee_level (IB, direction -1)
   ...
   Filter currency == 'USD' and fee_rate is not None. signal_raw = math.log1p(fee_rate / 100.0) — fee_rate is annualized PERCENT, heavily right-skewed with a very long tail, which is why the log1p is not optional.
...
F) etf_share_change (SSGA, direction +1)
   ...
   security_id = f"CUSIP:{Identifier}" — in-file, permanent, PIT-clean.
